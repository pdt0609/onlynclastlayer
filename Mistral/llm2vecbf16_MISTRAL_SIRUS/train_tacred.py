...
from losses import MutualInformationLoss, HardSoftMarginTripletLoss, HardMarginLoss , TripletLoss
...
class Manager(object):
    ...
    def get_cluster_and_centroids(self, embeddings):
        ...
        clustering_model = AgglomerativeClustering(n_clusters=None,metric="cosine",linkage="average", distance_threshold=0.5).fit(embeddings)
        ...
        for cluster_id in np.unique(clusters):
            if cluster_id not in centroids:
                ...
                # Calculate the centroid using PyTorch mean function
                centroid = torch.mean(cluster_embeddings, dim=0)
                
                # Store the centroid in the dictionary with cluster_id as the key
            ...
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                for hid in hidden:
                    ...
                    try:
                        ...
                        # Fallback to top 1 if an error occurs
                        top2_centroids = relationid2_clustercentroids[labels[torch.argmax(cos_similarities).item()].item()]
                ...
                loss4 = triplet(hidden, rep_des, nearest_cluster_centroids)
                ...
                loss2 = self.moment.contrastive_loss_des(rep_des, hidden, labels, is_memory)

                loss3 = triplet(hidden, rep_des,  cluster_centroids)

                loss4 = triplet(hidden, rep_des, nearest_cluster_centroids)

                # loss = loss1 + 2*loss2 + loss3 + loss4
                # loss = loss1 + 2*loss2 + 0.1*loss3 + 0.1*loss4
                loss = loss1 + 2*loss2 + 1*loss3 + 1*loss4
    ...
    def train(self):
        ...
        for step, (training_data, valid_data, test_data, current_relations, \
            ...
            # clusters, clusters_centroids = self.get_cluster_and_centroids(rep_des)
            ...
            # for i in range(len(seen_relations)):
            #     relation_2_cluster[self.rel2id[seen_relations[i]]] = clusters[i]
            ...
            #     self.train_model(encoder, memory_data_initialize, seen_des, relation_2_cluster, is_memory=True)
